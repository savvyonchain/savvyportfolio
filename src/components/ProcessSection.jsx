...
// Container orchestrates children one by one
const listVariants = {
	hidden: {},
	show: { transition: { staggerChildren: 0.3, delayChildren: 0.05 } },
}

// Each step slides up and fades in
const itemVariants = {
...
export default function ProcessSection() {
	...
	useEffect(() => {
		const fetchSteps = async () => {
			...
			setSteps((data || []).sort((a, b) => Number(a.step_number) - Number(b.step_number)))
		}
		fetchSteps()
	}, [])
			...
			<div className='rounded-[2rem] sm:rounded-[3.5rem] bg-white/[0.03] backdrop-blur-3xl border border-white/10 overflow-hidden flex flex-col lg:flex-row shadow-[0_40px_100px_rgba(0,0,0,0.6)] relative group min-w-0'>
				...
				<div className='lg:w-1/3 relative min-h-[400px] lg:min-h-full overflow-hidden bg-[#050308] border-r border-white/5'>
					...
					<div className='absolute inset-0 flex flex-col items-center justify-center p-6 sm:p-12 text-center'>
						...
						<h2
							className='text-3xl sm:text-4xl md:text-6xl font-black text-white leading-none tracking-tighter uppercase mb-2 break-words max-w-full px-2'
							style={{ fontFamily: 'var(--font-space)' }}
						>
							My <br />
							<span className='text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-brand)] to-[#d484ff] font-black'>
								Process
							</span>
						</h2>
						<p className='text-[10px] text-gray-500 font-bold uppercase tracking-[0.4em] mt-4'>
				...
				<div className='lg:w-2/3 p-6 sm:p-10 lg:p-20 relative bg-gradient-to-br from-white/[0.02] to-transparent min-w-0'>
					{/* Single viewport observer on the list drives the stagger */}
					<motion.div
						className='space-y-4'
							...
							>
								<div className='flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 lg:gap-10'>
									<div className='relative flex items-center justify-center sm:justify-start shrink-0'>
										<span
											className='text-4xl sm:text-5xl font-black text-white/5 group-hover:text-[var(--color-brand)]/30 transition-colors duration-500 select-none'
											style={{ fontFamily: 'var(--font-space)' }}
										>
											{prc.step_number}
										</span>
